...
class EventProcessor:
    ...
    def mass(self, event):
        ...
        if (event.totalWeight > 2):
            print(f"Detected weight: {event.totalWeight} kg")  # Print detected weight
...
class Wiiboard:
    ...
    def connect(self, address):
        ...
        if self.receivesocket and self.controlsocket:
            ...
            print (f"Starting to receive data from {address}...")
        else:
    ...
    def receive(self):
        while self.status == "Connected" and not self.processor.done:
            ...
            print(f"Raw data received: {data.hex()}")  # Print raw hex representation of the data
    ...
    def discover(self):
        print ("Pressing the red sync button on the board now")
        #----------GPIO Setup/Set for Sync Button Relay Control---------
        ...
        for bluetoothdevice in bluetoothdevices:
            #...
            print(f"Device Name: {bluetoothdevice[1]}, Address: {bluetoothdevice[0]}")  # Print all discovered devices
            #...
    ...
    def createBoardEvent(self, bytes):
        #print statements for debugging - delete after
        if len(bytes) < 12:
        	print("Not enough bytes received to create BoardEvent. Length received:", len(bytes))
        return BoardEvent(0, 0, 0, 0, False, False)  # Return a default BoardEvent
        #...........................
        buttonBytes = bytes[0:2]
        bytes = bytes[2:12]
        #... 
        print(f"Button Bytes: {buttonBytes.hex()}, Data Bytes: {bytes.hex()}")  # Debugging the bytes
        #...
        buttonPressed = False
